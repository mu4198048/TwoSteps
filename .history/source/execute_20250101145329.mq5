    ...
    if(tradeStrategy == EnumToString(Enum_OrderType=BUY)){
        if(authed){
            ...
            if(bid < twoFactorEntryPrice){
                ...
                Pirnt("Follow entries and pullback entries are valid.");
            }else{
                if(bid < buyEntryPrice){
                    isFollowEntryAvailable = true;
                Pirnt("Follow entry is valid.");
                Pirnt("The pullback entry is invalid because it exceeds the entry price.");
                }
            }
            if(! isTwoFactorEntryAvailable && ! isFollowEntryAvailable){
                Pirnt("Both the follow entry and the pullback entry are invalid because they exceed the entry price.");
            }
        }else
        if(! authed){
