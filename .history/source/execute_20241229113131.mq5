...
int OnInit(){
    ...
    getMagicNumber(_broker);getTradeStrategy();GetDetailsFollow(priceDataArr, 14);if(tradeStrategy == EnumToString(Enum_OrderType=BUY)){    firstFactoPrice = priceDataArr[0];    twoFactoEntryPrice = priceDataArr[1];    twoFactoLcPrice = priceDataArr[2];    twoFactoTpPrice = priceDataArr[3];    buyEntryPrice = priceDataArr[4];    lcPrice = priceDataArr[6];    tpPrice = priceDataArr[7];}elseif(tradeStrategy == EnumToString(Enum_OrderType=SELL)){    firstFactoPrice = priceDataArr[0];    twoFactoEntryPrice = priceDataArr[1];    twoFactoLcPrice = priceDataArr[2];    twoFactoTpPrice = priceDataArr[3];    sellEntryPrice = priceDataArr[5];    lcPrice = priceDataArr[6];    tpPrice = priceDataArr[7];}elseif(tradeStrategy == EnumToString(Enum_OrderType=BREAK)){    breakBuyPrice = priceDataArr[8];    breakBuyLcPrice = priceDataArr[10];    breakBuyTpPrice = priceDataArr[11];    breakSellPrice = priceDataArr[9];    breakSellLcPrice = priceDataArr[12];    breakSellTpPrice = priceDataArr[13];}
