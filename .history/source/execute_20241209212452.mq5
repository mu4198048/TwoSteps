...
void getTradeStrategy(){
    if(ObjectFind(0, trendLinesName[4])){tradeStrategy = EnumToString(Enum_OrderType=BUY);}else
    if(ObjectFind(0, trendLinesName[5])){tradeStrategy = EnumToString(Enum_OrderType=SELL);}else
    if(ObjectFind(0, trendLinesName[8] || ObjectFind(0, trendLinesName[9])){tradeStrategy = EnumToString(Enum_OrderType=SELL);}else
}