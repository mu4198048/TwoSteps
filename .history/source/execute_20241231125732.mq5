...
input double maximumLossPips = 100.0;  //Maximum Loss pips
...
void OnTick(){
    if(Hour() >= tradeStartHour && Hour() < tradeEndHour){//!==OnTime==
        ...
        if(_ordersTotal() == 0){//?==entry==
            ...
            if(tradeStrategy == EnumToString(Enum_OrderType=SELL)){openSellOrder();}else
            if(tradeStrategy == EnumToString(Enum_OrderType=BREAK)){openBreakOrder();}
        }else{//?==exit==
...
double lotSizeFollow(double loss){
    return CalculateLotSize(initialBalance, allowedRisk, loss, _symbol);
}
double lotSizePullBack(double loss){
    return CalculateLotSize(initialBalance, allowedRisk, loss, _symbol);
}
double lotSizeBreakBuy(double loss){
    return CalculateLotSize(initialBalance, allowedRisk, loss, _symbol);
}
double lotSizeBreakSell(double loss){
    return CalculateLotSize(initialBalance, allowedRisk, loss, _symbol);
}