                    ...
                    if(openBreakOrder()){
                        if(isEnvironmentSuitable(maxRetries, allowedGap, allowedSpread, convertPipsToPrice)){
                            if(comment == EnumToString(Enum_Comment = UPSIDE_BREAK)){
                                ...
                                if(! isDataRetrieved){
                                    if(_ordersTotal() == 0){
                                        if(comment == EnumToString(Enum_Comment = FOLLOW)){isBreakBuyEntryAvailable = false;}else
                                        if(comment == EnumToString(Enum_Comment = PULL_BACK)){isTwoFactorEntryAvailable = false;}L
                                    }
                                }
                            }else
