...
input double maximumLossPips = 115.0;  //Maximum Loss pips
                ...
                if(isEnvironmentSuitable(maxRetries, allowedGap, allowedSpread, convertPipsToPrice)){
                    if(openSellOrder()){
                        if(comment == EnumToString(Enum_Comment = FOLLOW)){
                            positionTicket = _orderSendBuy(lossPips+5.0, adjustedLotSize, magicNumber, positionComment, convertPipsToPrice);
                            }
                        }else
                        if(comment == EnumToString(Enum_Comment = PULL_BACK)){}
                    }
