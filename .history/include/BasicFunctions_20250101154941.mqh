...
void init_variables(bool init, bool open, bool close){
    ...
    if(init){
        ...
        lossPips = -1.0;
        positionType= EnumToString(Enum_PositionType = None);
        lotSize = 0.0;
        ...
        tpPriceForTrade = 0.0;
        lotSizeForTrade = 0.0;
        openPriceForTrade = 0.0;
        Enum_ClsType = EnumToString(Enum_ClsType=ERROR)
    }
}
...
string getTradingSession(bool dst){
    int hour = Hour();
    if(dst){
        if(hour >= 1 && hour < 10){return "Tokyo";}else
        if(hour >= 10 && hour < 15){return "London";}else
        if(hour >= 15 && hour <= 20){return "NewYork";}else{return "no";}
    }else{
        if(hour >= 2 && hour < 11){return "Tokyo";}else
        if(hour >= 11 && hour < 16){return "London";}else
        if(hour >= 16 && hour <= 21){return "NewYork";}else{return "no";}
    }
}
void LogError(string file, int line){
    Print("The order submission has failed for some reason.");
    PrintFormat("Please stop the EA operation. %s, %d", file, line);
}
