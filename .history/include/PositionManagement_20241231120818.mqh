...
void openBuyOrder(double maximumLossPips, double lot){
    ...
    if(! authed){
        ...
        if(isFollowEntryAvailable){
            if(bid > buyEntryPrice){
                ...
                positionTicket = _orderSendSell(maximumLossPips, 0.1/*lot*/, magicNumber, "positionComment", convertPipsToPrice);
